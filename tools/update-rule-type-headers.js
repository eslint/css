 ...
 * E.g. running with CLI arguments `"./src/rules/no-duplicate-imports.js"`
 * will only update the TSDoc header comment for the `no-duplicate-imports` rule.
...
import ruleVersions from "../docs/data/rule-versions.json" with { type: "json" };
 ...
 */
async function getConsideredRuleIds(args) {
	let ruleIds;

	if (args.length) {
		ruleIds = args
			.filter(arg => path.relative(arg, rulesDir) === "..")
			.map(ruleFile => path.basename(ruleFile, ".js"));
	} else {
		ruleIds = (await fs.readdir(rulesDir)).map(ruleFile =>
			path.basename(ruleFile, ".js"),
		);
	}
 ...
 */
async function createTSDoc(ruleId) {
	const ruleURL = pathToFileURL(path.resolve(rulesDir, `${ruleId}.js`));
	const { default: rule } = await import(ruleURL);
	const ruleMeta = rule.meta;
	const ruleDocs = ruleMeta.docs;
	const since = ruleVersions.added[ruleId];
	const lines = [
...
async function updateTypeDeclaration(ruleTypeFile, consideredRuleIds, check) {
	const sourceText = await fs.readFile(ruleTypeFile, "utf-8");
	...
	for (const [
		...
		const tsDoc = await createTSDoc(ruleId);
	...
	if (newSourceText !== sourceText) {
		...
		await fs.writeFile(ruleTypeFile, newSourceText);
	}
...
(async () => {
	...
	const consideredRuleIds = await getConsideredRuleIds(args);
	const ruleTypeFile = path.resolve(dirname, "../src/types/rules.d.ts");
